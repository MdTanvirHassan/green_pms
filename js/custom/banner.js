$(document).ready(function () {
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/banner/load_banner_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/banner/delete_bannerdata",
    ...
    $("#project-table tbody").sortable({
        ...
        update: function(event, ui) {
            ...
            $('#project-table input#sorting').each(function(i){
                sortinglist = sortinglist + ',' + $(this).val();
                
            });
            $.getJSON('backend/banner/sorting_bannerdata', {
                method:'post',
                returnFormat:'JSON',
...
$('div#myModal input#add').live('click',function(e){
    ...
    if($('div#myModal input#title').val()==''){
        bootbox.alert('Please Enter Banner title', function() { });
        return false;
        error = 1;
    }
    
    if($('div#myModal span#image').html()==''){
        bootbox.alert('Please upload Banner image', function() { });
        return false;
        error = 1;
    }
    if($('div#myModal div.note-editor .note-editable').html()==''){
        bootbox.alert('Please Enter Banner Description', function() { });
        return false;
        error = 1;
    }
    if(!error){
...
$('a.umodal').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "backend/banner/edit_banner",
        data: "edit_id=" + splitedArray[1],
        dataType: "json",
        success: function (data) {
            $(data).each(function (i, v) {
                $('div#updatemyModal div.modal-body input#title').val(v.banner_title);
                $('div#updatemyModal div.modal-body input#image').attr('src','images/banner_image/thumbnail/'+v.banner_image);
                $('div#updatemyModal div.modal-body input#bannerId').val(v.id);
                $('div#updatemyModal div.modal-body input#imageId').val(v.banner_image);
                $('div#updatemyModal div.note-editor .note-editable').html(v.banner_description);
