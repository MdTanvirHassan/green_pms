...
$('div.summernote').summernote();
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/category/load_parentCatecory_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/category/delete_parentCategorydata",
...
$('div#myModal input#add').live('click',function(e){
    var error = 0;
    if($('div#myModal input#cat_name').val()==''){
        bootbox.alert('Please Enter Category Name', function() { });
        return false;
        error = 1;
    }
    if($('div#myModal div.note-editor .note-editable').html()==''){
        bootbox.alert('Please Enter Category Description', function() { });
        return false;
        error = 1;
    }
    ...
    if(!error){
        ...
        $.post('backend/category/addParentCategory', {
            ...
            $('div#myModal input#cat_name').val('');
...
$('a.umodal').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "backend/category/edit_parentCategory",
        data: "edit_id=" + splitedArray[1],
        dataType: "json",
        success: function (data) {
            $(data).each(function (i, v) {
                $('div#updatemyModal div.modal-body input#cat_name').val(v.cat_name);
                $('div#updatemyModal div.modal-body input#cat_id').val(v.cat_id);
               $('div#updatemyModal div.note-editor .note-editable').html(v.category_description);
                if(v.is_active==1){
...
$('div#updatemyModal input#update').live('click', function (e) {
    var error = 0;
    if($('div#updatemyModal input#cat_name').val()==''){
        bootbox.alert('Please Enter Category Name', function() { });
        return false;
        error = 1;
    }
    ...
    if(!error){
        e.preventDefault();
        $.ajax({
            url: 'backend/category/updateParentCategory',
            async: false,
            dataType: "json",
            data: {
                cat_name: $.trim($('div#updatemyModal div.modal-body input#cat_name').val()),
                cat_desc: $.trim($('div#updatemyModal div.note-editor .note-editable').html()),
                cat_id : $('div#updatemyModal div.modal-body input#cat_id').val(),
            ...
            success: function (data) {
                ...
                $('div#updatemyModal input#cat_name').val('');
