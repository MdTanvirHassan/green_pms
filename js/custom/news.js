$(document).ready(function () {
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/news/load_news_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/news/delete_newsdata",
...
$('div#myModal input#add').live('click',function(e){
    ...
    if($('div#myModal input#title').val()==''){
        bootbox.alert('Please Enter News title', function() { });
        return false;
        error = 1;
    }

    if($('div#myModal div.note-editor .note-editable').html()==''){
        bootbox.alert('Please Enter News Description', function() { });
        return false;
        error = 1;
    }
    if(!error){
...
$('a.umodal').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "backend/news/edit_news",
        data: "edit_id=" + splitedArray[1],
        dataType: "json",
        success: function (data) {
            var string = '';
            $(data.image).each(function (i, v) {
                string += '<div class="image_'+v.id+'" style="height:50px;width:50px;float:left; margin:2px;"><img id="main_image_'+v.id+'" src="images/news_image/'+v.image+'" style="height:50px;width:50px;float:left;"/><center><img style="float:left; cursor:pointer;margin-top: -100%;" src="images/delete_red.png" id="delete_image_'+v.id+'"/></center></div>'
            });
            ...
            $(data.data).each(function (i, v) {
                ...
                $('div#updatemyModal div.modal-body input#newsId').val(v.id);
    ...
    if(!error){
        ...
        $.ajax({
            ...
            data: {
                ...
                id : $('div#updatemyModal div.modal-body input#newsId').val()
            },
            type: "POST",
            success: function (data) {
...
$('div#updatemyModal img[id*="delete_image_"]').live('click',function(e){
    ...
    $.ajax({
        type: "POST",
        url: "backend/news/delete_single_image",
        data: {
                id: $.trim(id[2]),
                news_id : $('div#updatemyModal div.modal-body input#newsId').val()
            },
        success: function (msg) {
           var data = msg.split('"');
           $('div#updatemyModal div.image_'+data[1]).remove();
