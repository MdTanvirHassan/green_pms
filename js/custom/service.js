...
$('div.summernote').summernote();
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/service/load_service_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/service/delete_servicedata",
...
$('div#myModal input#add').live('click',function(e){
    ...
    if($('div#myModal input#name').val()==''){
        bootbox.alert('Please Enter Service Name', function() { });
        return false;
        error = 1;
    }
    if($('div#myModal div.note-editor .note-editable').html()==''){
        bootbox.alert('Please Enter Service Description', function() { });
        return false;
        error = 1;
    }
    ...
    if(!error){
        ...
        $.post('backend/service/addService', {
            data: newArray
        }, function (data) {
...
$('div#updatemyModal input#update').live('click', function (e) {
    var error = 0;
    if($('div#updatemyModal input#cname').val()==''){
        bootbox.alert('Please Enter Service Name', function() { });
        return false;
        error = 1;
    }
    ...
    if(!error){
        e.preventDefault();
        $.ajax({
            url: 'backend/service/updateService',
            async: false,
            dataType: "json",
            data: {
                ...
                id : $('div#updatemyModal div.modal-body input#id').val()
            },
            type: "POST",
            success: function (data) {
                ...
                $('div#updatemyModal input#name').val('');
