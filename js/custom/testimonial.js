...
$('div.summernote').summernote();
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/testimonial/load_testimonial_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/testimonial/delete_testimonialdata",
...
$('div#myModal input#add').live('click',function(e){
    var error = 0;
    if($('div#myModal input#name').val()==''){
        bootbox.alert('Please Enter Testimonial Name', function() { });
        return false;
        error = 1;
    }

    if($('div#myModal div.note-editor .note-editable').html()==''){
        bootbox.alert('Please Enter Testimonial', function() { });
        return false;
        error = 1;
    }
    if(!error){
        e.preventDefault();
        var  newArray  = {
            name:$('div#myModal input#name').val(),
            testi:$('div#myModal div.note-editor .note-editable').html()
        };
        
        $.post('backend/testimonial/addTestimonial', {
            data: newArray
        }, function (data) {
...
$('a.umodal').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "backend/testimonial/edit_testimonial",
        data: "edit_id=" + splitedArray[1],
        dataType: "json",
        success: function (data) {
            $(data).each(function (i, v) {
                $('div#updatemyModal div.modal-body input#name').val(v.name);
                $('div#updatemyModal div.modal-body input#testimonialId').val(v.id);
                $('div#updatemyModal div.note-editor .note-editable').html(v.testi);
...
$('div#updatemyModal input#update').live('click', function (e) {
    var error = 0;
    if($('div#updatemyModal input#name').val()==''){
        bootbox.alert('Please Enter Testimonial Name', function() { });
        return false;
        error = 1;
    }
    ...
    if(!error){
        e.preventDefault();
        $.ajax({
            url: 'backend/testimonial/updateTestimonial',
            async: false,
            dataType: "json",
            data: {
                name: $.trim($('div#updatemyModal div.modal-body input#name').val()),
                testi: $.trim($('div#updatemyModal div.note-editor .note-editable').html()),
                id : $('div#updatemyModal div.modal-body input#testimonialId').val()
            },
            type: "POST",
            success: function (data) {
