...
$('div.summernote').summernote();
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/description/load_description_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/description/delete_descriptiondata",
...
$('div#myModal input#add').live('click',function(e){
    var error = 0;
    if($('div#myModal div#heading div.note-editable').html()==''){
        bootbox.alert('Please Enter title', function() { });
        return false;
        error = 1;
    }

    if($('div#myModal div#description div.note-editable').html()==''){
        bootbox.alert('Please Enter Description', function() { });
        return false;
        error = 1;
    }
    if(!error){
        ...
        var  newArray  = {
            title:$('div#myModal div.#heading div.note-editable').html(),
            description:$('div#myModal div#description div.note-editable').html()
        };
        
        $.post('backend/description/addDescription', {
            data: newArray
        }, function (data) {
            ...
            $('div#myModal div#heading div.note-editable').html('');
            $('div#myModal div#description div.note-editable').html('');
            oTable.fnDraw(false);
        },'JSON');
    }
})
...
$('a.umodal').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "backend/description/edit_description",
        data: "edit_id=" + splitedArray[1],
        dataType: "json",
        success: function (data) {
            $(data).each(function (i, v) {
                $('div#updatemyModal div#heading div.note-editable').html(v.title);
                $('div#updatemyModal div.modal-body input#descriptionId').val(v.id);
                $('div#updatemyModal div#description div.note-editable').html(v.description)
...
$('div#updatemyModal input#update').live('click', function (e) {
    var error = 0;
    if($('div#updatemyModal div#heading div.note-editable').html()==''){
        bootbox.alert('Please Enter title', function() { });
        return false;
        error = 1;
    }
  
    if($('div#updatemyModal div#description div.note-editable').html()==''){
        bootbox.alert('Please Enter Description', function() { });
        return false;
        error = 1;
    }
    if(!error){
        e.preventDefault();
        $.ajax({
            url: 'backend/description/updateDescription',
            async: false,
            dataType: "json",
            data: {
                title: $.trim($('div#updatemyModal div#heading div.note-editable').html()),
                description: $.trim($('div#updatemyModal div#description div.note-editable').html()),
                id : $('div#updatemyModal div.modal-body input#descriptionId').val()
            },
            type: "POST",
            success: function (data) {
               ...
               $('div#updatemyModal div#heading div.note-editable').html('');
               $('div#updatemyModal div#description div.note-editable').html('');
