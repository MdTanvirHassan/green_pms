...
$('div.summernote').summernote();
    ...
    tableElement.dataTable({
        ...
        "bServerSide": true,
        "sServerMethod": "GET",
        "sAjaxSource": "backend/about_us/load_about_us_data",
        "iDisplayLength": 10,
        "aLengthMenu": [
        [10, 25, 50, -1],
        [10, 25, 50, "All"]
            ...
            $("a[id*='delete_']").click(function () {
                ...
                bootbox.confirm("Do you confirm to Delete!", function(r) {
                    if(r){
                        $.ajax({
                            type: "POST",
                            url: "backend/about_us/delete_about_usdata",
...
$('div#myModal input#add').live('click',function(e){
    ...
    if(!error){
        ...
        var  newArray  = {
            ...
            details:$('div#myModal div#description div.note-editable').html()
        };
        
        $.post('backend/about_us/addAbout_us', {
            data: newArray
        }, function (data) {
...
$('a.umodal').live('click', function () {
    ...
    $.ajax({
        type: "POST",
        url: "backend/about_us/edit_about_us",
        data: "edit_id=" + splitedArray[1],
        dataType: "json",
        success: function (data) {
            $(data).each(function (i, v) {
                $('div#updatemyModal div#heading div.note-editable').html(v.title);
                $('div#updatemyModal div.modal-body input#about_usId').val(v.id);
                $('div#updatemyModal div#description div.note-editable').html(v.details)
...
$('div#updatemyModal input#update').live('click', function (e) {
    ...
    if(!error){
        e.preventDefault();
        $.ajax({
            url: 'backend/about_us/updateAbout_us',
            async: false,
            dataType: "json",
            data: {
                title: $.trim($('div#updatemyModal div#heading div.note-editable').html()),
                details: $.trim($('div#updatemyModal div#description div.note-editable').html()),
                id : $('div#updatemyModal div.modal-body input#about_usId').val()
            },
            type: "POST",
            success: function (data) {
