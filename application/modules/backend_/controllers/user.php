...
left join users u on u.user_id=dt.moderator where t.project_id=$project_id and t.task_id=$task_id and dt.dept_id=$dept_id and t.is_active=1";
    ...
    function set_moderator() {
        ...
        if (!empty($res)) {
            $this->m_common->update_row('department_task', array('dept_task_id' => $res[0]['dept_task_id']), array('moderator' => $moderator, 'start_date' => $start_date, 'end_date' => $end_date));
            $user = $this->m_common->get_row_array('users', array('user_id' => $moderator), 'username');
            echo json_encode(array('status' => 'success', 'data' => $user[0]['username']));
        } else {
            $this->m_common->insert_row('department_task', array('moderator' => $moderator, 'start_date' => $start_date, 'end_date' => $end_date, 'task_id' => $task_id, 'dept_id' => $dept_id, 'project_id' => $project_id));
            $user = $this->m_common->get_row_array('users', array('user_id' => $moderator), 'username');
            echo json_encode(array('status' => 'success', 'data' => $user[0]['username']));
        }
    }
