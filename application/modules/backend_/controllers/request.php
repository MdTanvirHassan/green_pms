...
join task tsk on dts.task_id=tsk.task_id join users u on r.applicant_id=u.user_id where r.approver_id=$user_id and r.status='pending'";
    ...
    function approveResetStatus(){
        ...
        if (!empty($result)) {
            $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id= $task_id and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
            $tall = $this->m_common->customeQuery($sql);
            $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id= $task_id and status = 'complete' and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
            ...
            if (!empty($parent[0]['sub_task_id'])) {
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(select GROUP_CONCAT(task_id) as task_id from task where sub_task_id=" . $parent[0]['sub_task_id'] . ") and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
                $pall = $this->m_common->customeQuery($sql);
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(select GROUP_CONCAT(task_id) as task_id from task where sub_task_id=" . $parent[0]['sub_task_id'] . ") and status = 'complete' and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
            ...
            if (!empty($parent[0]['parent_task_id'])) {
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(select GROUP_CONCAT(task_id) as task_id from task where parent_task_id=" . $parent[0]['parent_task_id'] . ") and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
                $pall = $this->m_common->customeQuery($sql);
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(select GROUP_CONCAT(task_id) as task_id from task where parent_task_id=" . $parent[0]['parent_task_id'] . ") and status = 'complete' and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
                $pcomplete = $this->m_common->customeQuery($sql);
                $p_percentage = ($pcomplete[0]['total'] * 100) / $pall[0]['total'];
                $p_percentage = empty($p_percentage) ? 0:$p_percentage;
