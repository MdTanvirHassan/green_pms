...
left join users u on u.user_id=dt.moderator where t.project_id=$project_id and dt.dept_id=$dept_id and t.is_active=1";
            ...
            $parent_task_sql = "select task.*,dt.start_date,dt.end_date from task join department_task dt on task.task_id=dt.task_id where dt.project_id=" . $project_id . " and dt.dept_id=" . $dept_id . " and task.parent_task_id is null and task.sub_task_id is null";
...
left join users u on u.user_id=dt.moderator where t.project_id=$project_id and dt.dept_id=$dept_id and t.is_active=1";
    ...
    function taskViewDetails() {
        ...
        $dept_id = $this->input->post('dept_id');
        $data['tasks'] = $this->m_common->get_row_array('task', array('task_id' => $task_id, 'is_active' => 1), '*');
        if (empty($data['tasks'][0]['parent_task_id']) && empty($data['tasks'][0]['sub_task_id'])) {
            ...
            $data['tasks'] = $this->m_common->get_row_array('task', array('task_id' => $data['tasks'][0]['parent_task_id'], 'is_active' => 1), '*');
            foreach ($data['tasks'] as $key => $value) {
                $e_t = $this->m_common->get_row_array('department_task', array('task_id' => $value['task_id'], 'dept_id' => $dept_id, 'project_id' => $value['project_id']), 'percentage');
                $data['tasks'][$key]['percentage'] = !empty($e_t) ? round($e_t[0]['percentage'], 2) : 0;
                $data['tasks'][$key]['sub_tasks'] = $this->m_common->get_row_array('task', array('parent_task_id' => $value['task_id'], 'sub_task_id' => null, 'is_active' => 1), '*');
    ...
    function completeStatus() {
        ...
        if (!empty($result)) {
            ...
            if (!empty($parent[0]['sub_task_id'])) {
                ...
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(" . $tasks . ") and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
                $pall = $this->m_common->customeQuery($sql);
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(" . $tasks . ") and status = 'complete' and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
            ...
            if (!empty($parent[0]['parent_task_id'])) {
                $tasks = $this->m_common->customeQuery("select GROUP_CONCAT(task_id) as task_id from task where is_active=1 and sub_task_id is not NULL and parent_task_id=" . $parent[0]['parent_task_id'] . "");
                if (!empty($tasks)) {
                    $tasks = $tasks[0]['task_id'];
                }
                $sql = "select sum(percentage) as total from department_task_status where is_active=1 and task_id in(" . $tasks . ") and project_id=" . $exists[0]['project_id'] . " and dept_id=" . $exists[0]['dept_id'] . "";
...
//            rename($old_name, $new_name);
            ...
            echo json_encode(array('status' => 'success', 'temp_file_path' => $file_path, 'file_name' => $fileRealName, 'file_id' => $result));
        }
    }
