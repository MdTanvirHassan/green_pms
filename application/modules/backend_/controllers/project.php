        ...
        if (!$this->is_logged_in($this->session->userdata('logged_in'))) {
           ...
           // $data['projects'] = $this->m_common->get_row_array('projects', array('is_active' => 1), '*');
            if(!empty($order_by)){
                ...
                $data['projects'] = $this->m_common->get_row_array('projects', array('is_active' => 1), '*','','','position');
                $data['order_by']='';
            }
...
//                }
    ...
    function importProject(){
        ...
        $this->load->view('project/v_import_project');
    }
...
//             exit;
    ...
    function taskInfo($id = "") {
        ...
        $project_id = $postData['project_id'];
        $data["task_info"] = $this->m_common->get_row_array('task', array('task_id' => $task_id), '*');
    ...
    function editsubTaskInfo($id = "") {
        ...
        $data["parent_task_info"] = $this->m_common->get_row_array('task', array('task_id' => $data["task_info"][0]['parent_task_id']), '*');
    ...
    function editrootTaskInfo($id = "") {
        ...
        $data["parent_task_info"] = $this->m_common->get_row_array('task', array('task_id' => $data["task_info"][0]['parent_task_id']), '*');
        $data["sub_task_info"] = $this->m_common->get_row_array('task', array('task_id' => $data["task_info"][0]['sub_task_id']), '*');
    ...
    function taskSubtaskInfo($id = "") {
        ...
        $data["task_info"] = $this->m_common->get_row_array('task', array('task_id' => $task_id), '*');
        $data["sub_task_info"] = $this->m_common->get_row_array('task', array('task_id' => $sub_task_id), '*');
        $test = $this->m_common->get_row_array('task', array('task_id' => $sub_task_id), '*');
        // $data["parent_task_info"] = $this->m_common->get_row_array('task', array('task_id' =>$parent_task_id), '*');
    ...
    function addEditSubTask($id = "") {
        ...
        $insertData['created_by'] = $user_id;
        $insertData['created'] = date('Y-m-d');
        $pre_task=$this->m_common->get_row_array('task',array('project_id'=>$project_id,'task_name'=>$postData['task_name']),'*');
        if(empty($pre_task)){
            $result = $this->m_common->insert_row('task', $insertData);
        }else{
            $result='';
